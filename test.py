...
def test(test_dataframe):
    ...
    model3.load_state_dict(torch.load(Model_Path + "BEST_MODEL_ERGCNNPPIS.pkl", map_location='cuda'))
...
def test_one_dataset(dataset):
    ...
    for id in test_dataframe['ID']:
        try:
            get_node_features(id)
        except:
            excep.append(id)
    test_dataframe=test_dataframe[~test_dataframe['ID'].isin(excep)]
    test(test_dataframe)
