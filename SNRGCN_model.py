...
def get_node_features(sequence_name):
    ...
    SASA = np.load(Feature_Path + "SASA/" + sequence_name + '.npy').reshape(-1,1)
    HP = np.load(Feature_Path + "HP/" + sequence_name + '.npy').reshape(-1,1)
    EP = np.load(Feature_Path + "EP/" + sequence_name + '.npy').reshape(-1,1)
    node_features = np.concatenate(
        [dssp_feature,pssm_feature, RAA_feature, hmm_feature, anchor_feature, pro2Vec_feature, psaia_feature, SASA, EP, HP], axis=1)
